...
class Segment(Command):
    ...
    def add_to_subparsers(cls, subparsers):
        p = subparsers.add_parser(
            cls.NAME, help=cls.HELP, epilog=cls.EPILOG, formatter_class=formatter)
        ...
        add("--sat_source", metavar='\b', help=cls.HELP_SAT_SOURCE)
        add("--norad_to_receiver_tbl", metavar='\b', help=cls.HELP_NORAD)
        add("--sat_positions_tbl", metavar='\b', help=cls.HELP_SAT_POSITIONS)
        add("--sat_offset_dest", metavar='\b', help=cls.HELP_SAT_OFFSET_DEST)
        add("--bad_hour_padding", type=int, default=1, metavar='\b', help=cls.HELP_BAD_HOUR)
        add("--max_timing_offset_s", type=int, default=30, metavar='\b', help=cls.HELP_MAX_OFFSET)
        add("--date_range", metavar='\b', help=cls.HELP_DATE_RANGE)
        add("--wait_for_job", default=False, action="store_true", help=cls.HELP_WAIT)
        add("--segmenter_params", default="{}", metavar='\b', help=cls.HELP_SEGMENTER_PARAMS)
        add("--merge_params", default="{}", metavar='\b', help=cls.HELP_MERGE_PARAMS)
        add("--ssvid_filter_query", metavar='\b', help=cls.HELP_SSVID_FILTER)
        add("--bins_per_day", default=4, metavar='\b', help=cls.HELP_BINS_PER_DAY)

    @classmethod
    def run(cls, args, extra_args):
        logger.info("Running pipe segment...")
