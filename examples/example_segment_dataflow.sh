...
docker compose run --rm dev segment \
  ...
  --out_segmented_messages_table=world-fishing-827.${DATASET_OUT}.messages_segmented \
  --out_segments_table=world-fishing-827.${DATASET_OUT}.segments \
  --fragments_table=world-fishing-827.${DATASET_OUT}.fragments \
  --in_normalized_sat_offset_messages_table=world-fishing-827.pipe_ais_sources_v20220628.pipe_nmea_normalized_ \
  --out_sat_offsets_table=world-fishing-827.${DATASET_OUT}.satellite_timing_offsets \
  --in_norad_to_receiver_table=world-fishing-827.pipe_static.norad_to_receiver_v20230510 \
  --in_sat_positions_table=world-fishing-827.satellite_positions_v20190208.satellite_positions_one_second_resolution_ \
  --setup_file=./setup.py \
